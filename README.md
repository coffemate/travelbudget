...
│   └── clerk-neon-migration.md # Supabase -> Neon + Clerk 迁移说明
├── frontend/
│   ├── .env.example
│   ├── package.json
│   ├── vite.config.js
...
本项目已迁移到 **Clerk 认证 + Neon PostgreSQL**。
...
### 当前鉴权方式
- 前端使用 `@clerk/clerk-js` 进行注册、登录、登出和会话管理。
- 前端请求后端 API 时，会自动把 Clerk JWT 放入 `Authorization: Bearer <token>`。
- 后端通过 Clerk JWT 验证逻辑识别用户，并读取 `payload.sub` 作为当前用户 ID。
- API 业务层通过 `owner_id/created_by` 与鉴权后的 `req.user.id` 做用户数据隔离。
